...
complex problems, numerical methods, particularly the \textit{Finite Element
...
the space-time grid based on \textit{a posteriori} error indicators, a strategy extensively
discussed in the context of \textit{a posteriori} error estimates for parabolic
...
weak (\textit{variational}) formulation of the problem. This involves multiplying
...
properties are critical to guarantee that the problem is \textit{well-posed} and
...
\item
  ...
  $W = \{w\in L^2(0,T;H^1(\Omega)) \text{ and } \partial w/\partial t\in L^2(0,T;H^{-1}(\Omega))\}$
  \cite{picasso1998, quarteroni_numerical_models}. Moreover, the solution $u(\cdot,t)$ belongs to
...
$t^n$ and $t^{n+1}$. For an exhaustive and rigorous treatment of the stability and convergence properties of this family of methods applied to parabolic equations, we refer to Quarteroni \cite{quarteroni_numerical_models}. The scheme for advancing from $\mathbf{U}^n$ to
$\mathbf{U}^{n+1}$ is: \[
...
For $\theta=1$:
\begin{itemize}
\item The scheme is first-order accurate in time ($\mathcal{O}(\Delta t)$).
...
Detailed \textit{a posteriori} error estimates for the error $e = u - u_{h\tau}$ in the $L^2$ in time, $H^1$ in space norm are established in \cite{picasso1998}. The error estimator is comprised of three main contributions: $\eta$, $\epsilon$, and $\gamma$ \cite{picasso1998}.
...
Finally, the results confirm that the computational overhead introduced by the adaptive mechanics (specifically the \textit{a posteriori} evaluation of the \texttt{KellyErrorEstimator} and the state projection via \texttt{SolutionTransfer}) is entirely negligible when compared to the massive reduction in the number of linear CG iterations and matrix-vector operations. The adaptive framework is therefore not only theoretically sound but also computationally highly advantageous for localized transient phenomena.
...
\bibitem{quarteroni_numerical_models}
A. Quarteroni, \emph{Numerical Models for Differential Problems}, 2nd ed., MS\&A, Springer, 2014.
\end{thebibliography}
