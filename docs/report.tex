...
The numerical solver relies on the deal.II framework, exploiting distributed memory parallelism via the Message Passing Interface (MPI) and the Trilinos library for linear algebra operations. The computational approach accommodates accuracy requirements that vary significantly across the space and time domains. Unlike homogeneous solvers characterized by fixed grids and constant step sizes, this implementation requires dynamic reallocation of data structures and algebraic systems to support continuous adaptation.

\subsection{Temporal Adaptivity (k-adaptivity)}

To accurately capture the sequence of impulses localized in space and time, the temporal discretization relies on an adaptive Backward Euler scheme ($\theta = 1.0$). At each integration step, a heuristic time-step control evaluates the $L^2$-norm of the difference between the current approximation ($u^n$) and the preceding approximation ($u^{n-1}$). This difference serves as a practical, computationally inexpensive proxy for the temporal error parameter $\epsilon^n$ established in theoretical frameworks.

The step-size adjustment is governed by predefined upper and lower tolerance thresholds. If the discrepancy exceeds the maximum permissible limit (\texttt{tol\_time\_max}), the candidate step is immediately rejected. The step magnitude $\Delta t$ is halved, and the system matrix and right-hand side are reassembled at the appropriate temporal coordinate. Conversely, if the estimated error is strictly below the minimum threshold (\texttt{tol\_time\_min}), the subsequent time step is enlarged by a factor of 1.2, subject to an upper bound (\texttt{dt\_max}). This prevents wasteful computations during quiescent periods of the domain evolution. 

The rejection and progression logic is expressed in the code as follows:
...
while (!step_accepted) {
    ...
    // Compute temporal error estimate via L2 norm of the difference
    TrilinosWrappers::MPI::Vector diff = solution_owned;
    diff.add(-1.0, solution_owned_old);
    error_t = diff.l2_norm() / std::sqrt(dof_handler.n_dofs()); 
...
The solver addresses localized spatial variations through dynamic refinement and coarsening of the $P_r$ simplex elements. Instead of rigorously implementing the full analytical spatial residual $\eta_K$, the routine relies on the \texttt{KellyErrorEstimator} module provided by deal.II. This acts as a robust surrogate, evaluating the spatial error strictly through the inter-element jumps of the normal derivative. 

Mesh adaptation is scheduled at constant intervals (every 5 time steps). The error metric is collected cell-wise across the distributed triangulation, and the elements are subsequently flagged. A fixed-number criterion is enforced, dictating that the $30\%$ of cells displaying the highest error are flagged for refinement, while the lowest $10\%$ are designated for coarsening.

In contrast to solvers restricted to homogeneous grids, modifications to the underlying mesh require an explicit projection of the physical state from the prior grid configuration to the updated geometry. This data preservation is managed by the \texttt{SolutionTransfer} structure, which interpolates the existing nodal values onto the updated degrees of freedom before the sparsity patterns and algebraic vectors are fundamentally reconstructed. 

The essential operations governing grid modification and solution transfer are organized within the \texttt{refine\_mesh()} function:
